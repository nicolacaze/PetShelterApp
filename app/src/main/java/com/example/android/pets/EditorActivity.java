...
public class EditorActivity extends AppCompatActivity implements LoaderManager.LoaderCallbacks<Cursor> {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (mCurrentPetUri != null) {
            //If not null, we are on edit mode and set action bar to say "Edit Pet".
            setTitle(getString(R.string.editor_activity_title_edit_pet));
            getLoaderManager().initLoader(PET_LOADER, null, this);
        } else {
            //If null, we are on insert mode and set action bar title to say "Add a Pet".
    ...
    private void deletePet() {
        //Delete the current Pet selected if we are on "edit mode".
        if (mCurrentPetUri != null) {
    ...
    private void savePet() {
        ...
        //Make a check whether fields are all empty. If true, leave the method, do not save Pet.
        if (name.isEmpty() && breed.isEmpty() && weightString.isEmpty() && mGender == PetEntry.GENDER_UNKNOWN) {
            return;
        }

        //If weight field is left empty, we assign the value 0 as default.
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        switch (item.getItemId()) {
            ...
            case R.id.action_delete:
                //Display alert dialog message to confirm delete of the Pet.
    ...
    public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
        ...
        if (data.moveToFirst()) {
            ...
            int petNameColumn = data.getColumnIndex(PetEntry.COLUMN_PET_NAME);
            int petBreedColumn = data.getColumnIndex(PetEntry.COLUMN_PET_BREED);
            int petGenderColumn = data.getColumnIndex(PetEntry.COLUMN_PET_GENDER);
            int petWeightColumn = data.getColumnIndex(PetEntry.COLUMN_PET_WEIGHT);

            //Extract data from cursor
            String petName = data.getString(petNameColumn);
            String petBreed = data.getString(petBreedColumn);
            int gender = data.getInt(petGenderColumn);
            int petWeight = data.getInt(petWeightColumn);

            //Fill EditText field with retrieved data.
